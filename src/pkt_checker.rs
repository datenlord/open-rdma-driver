...
impl PacketCheckerContext {
    ...
    fn check_pkt_map(&self) -> ThreadFlag {
        ...
        for (rkey_with_qpn, map) in iter_maps.iter() {
            let (is_complete, is_out_of_order, dqpn) = {
            ...
            if is_complete {
                ...
                let command =
                    RespCommand::Acknowledge(RespAckCommand::new_ack(dqpn, Msn::default()));
...
mod tests {
    ...
    fn test_packet_checker() {
        ...
        let _packet_checker = PacketChecker::new(send_queue, recv_pkt_map.clone());
        let key = RKeyWithQpn::new(Key::new(1), Qpn::new(3));
        recv_pkt_map.write().unwrap().insert(
            key.clone(),
            RecvPktMap::new(2, Psn::new(1), Qpn::new(3)).into(),
        );
        recv_pkt_map.read().unwrap().get(&key).unwrap().lock().unwrap().insert(Psn::new(1));
        sleep(Duration::from_millis(1));
        assert!(matches!(recv_queue.try_recv(), Err(mpsc::TryRecvError::Empty)));
        recv_pkt_map.read().unwrap().get(&key).unwrap().lock().unwrap().insert(Psn::new(2));
