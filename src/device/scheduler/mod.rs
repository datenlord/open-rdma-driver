...
use crate::device::{ScatterGatherElement, ScatterGatherList, ToCardWorkRbDescRequest};

use super::{Pmtu, ToCardWorkRbDesc};
        ...
        let thread_handler = spawn(move || {
            loop{
                ...
                if let Some(desc) = desc {
                    let dqpn = match &desc {
                        ToCardWorkRbDesc::Request(req) => req.dqpn,
                    };
...
fn cut_from_sgl(
    mut length: u32,
    origin_sgl: &mut ScatterGatherList,
    origin_sgl_level: &mut u32,
) -> ScatterGatherList {
    let mut current_level = *origin_sgl_level as usize;
    let mut new_sgl = ScatterGatherList {
    ...
    while (current_level as u32) < origin_sgl.len {
        if origin_sgl.data[current_level].len >= length {
            let addr = origin_sgl.data[current_level].laddr;
            new_sgl.data[new_sgl_level] = ScatterGatherElement {
                laddr: addr,
                len: length,
                lkey: origin_sgl.data[current_level].lkey,
            };
            new_sgl.len = new_sgl_level as u32 + 1;
            origin_sgl.data[current_level].laddr += length as u64;
            origin_sgl.data[current_level].len -= length;
            if origin_sgl.data[current_level].len == 0 {
                current_level += 1;
            }
            *origin_sgl_level = current_level as u32;
            return new_sgl;
        } else {
            // check next level
            let addr = origin_sgl.data[current_level].laddr as *mut u8;
            new_sgl.data[new_sgl_level] = ScatterGatherElement {
                laddr: addr as u64,
...
fn recalculate_psn(desc: &ToCardWorkRbDescRequest, base_psn: u32) -> u32 {
    let pmtu = get_pmtu(&desc.pmtu);
    let first_packet_length = get_first_packet_length(desc.raddr, pmtu);
    // first packet psn = base_psn
    // so the total psn = base_psn + (desc.common_header.total_len - first_packet_length) / pmtu + 1
    let last_packet_psn =
        base_psn + (desc.common_header.total_len - first_packet_length).div_ceil(pmtu);
    last_packet_psn + 1
}
...
#[cfg(test)]
mod test {
    use std::{collections::LinkedList, net::Ipv4Addr, sync::Arc, thread::sleep};

    use crate::{device::ToCardWorkRbDescOpcode, ToCardWorkRbDesc};

    use super::{recalculate_psn, split_descriptor};
    ...
    fn test_cut_from_sgl() {
        let mut sgl = crate::device::ScatterGatherList {
        ...
        sgl.data[0] = crate::device::ScatterGatherElement {
            laddr: 0,
            len: 1024,
            lkey: [0; 4],
        };
        sgl.data[1] = crate::device::ScatterGatherElement {
            laddr: 2000,
            len: 1024,
            lkey: [0; 4],
        };
        let mut sgl_level = 0;
        let new_sgl = super::cut_from_sgl(512, &mut sgl, &mut sgl_level);
        assert_eq!(new_sgl.len, 1);
        assert_eq!(new_sgl.data[0].len, 512);
        assert_eq!(sgl.data[0].len, 512);
        assert_eq!(sgl.data[0].laddr, 512);

        let new_sgl = super::cut_from_sgl(1024, &mut sgl, &mut sgl_level);
        assert_eq!(new_sgl.len, 2);
        assert_eq!(new_sgl.data[0].laddr, 512);
        assert_eq!(new_sgl.data[0].len, 512);
        assert_eq!(new_sgl.data[1].laddr, 2000);
    ...
    fn test_helper_function_split_descriptor() {
        ...
        sgl.data[2] = crate::device::ScatterGatherElement {
            laddr: 0x20000,
            len: 1024 * 35, // 35K
            lkey: [3; 4],
        };
