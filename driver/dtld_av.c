...
{
	dtld_av_from_attr(rdma_ah_get_port_num(attr), av, attr);
	dtld_av_fill_ip_info(av, attr);
...
int dtld_av_chk_attr(struct dtld_dev *dtld, struct rdma_ah_attr *attr)
{
	...
	if (rdma_ah_get_ah_flags(attr) & IB_AH_GRH) {
		...
		if (type < RDMA_NETWORK_IPV4 ||
			...
			pr_warn("invalid network type for rdma_dtld = %d\n",
					type);
			return -EINVAL;
		}
	}
...
void dtld_av_fill_ip_info(struct dtld_av *av, struct rdma_ah_attr *attr)
{
	...
	case RDMA_NETWORK_IPV4:
		type = DTLD_NETWORK_TYPE_IPV4;
		break;
	case RDMA_NETWORK_IPV6:
		type = DTLD_NETWORK_TYPE_IPV6;
		break;
	default:
		/* not reached - checked in dtld_av_chk_attr */
		type = 0;
		break;
	}
...
struct dtld_av *dtld_get_av(struct dtld_pkt_info *pkt, struct dtld_ah **ahp)
{
	...
	if (ah_num) {
		...
		ah = dtld_pool_get_index(&pkt->dtld->ah_pool, ah_num);
		if (!ah) {
			pr_warn("Unable to find AH matching ah_num\n");
			return NULL;
		}

		if (dtld_ah_pd(ah) != pkt->qp->pd) {
