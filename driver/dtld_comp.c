...
{
	struct dtld_qp *qp = from_timer(qp, t, retrans_timer);
...
// 	if (must_sched != 0)
// 		dtld_counter_inc(SKB_TO_PKT(skb)->dtld, DTLD_CNT_COMPLETER_SCHED);

// 	dtld_run_task(&qp->comp.task, must_sched);
// }

// static inline enum comp_state get_wqe(struct dtld_qp *qp,
// 				      struct dtld_pkt_info *pkt,
// 				      struct dtld_send_wqe **wqe_p)
// {
...
// static inline void reset_retry_counters(struct dtld_qp *qp)
// {
...
// static inline enum comp_state check_psn(struct dtld_qp *qp,
// 					struct dtld_pkt_info *pkt,
// 					struct dtld_send_wqe *wqe)
// {
// 	s32 diff;
...
// static inline enum comp_state check_ack(struct dtld_qp *qp,
// 					struct dtld_pkt_info *pkt,
// 					struct dtld_send_wqe *wqe)
// {
...
// 		case AETH_RNR_NAK:
// 			dtld_counter_inc(dtld, DTLD_CNT_RCV_RNR);
...
// 					dtld_counter_inc(dtld,
// 							DTLD_CNT_RCV_SEQ_ERR);
// 					qp->comp.psn = pkt->psn;
// 					if (qp->req.wait_psn) {
// 						qp->req.wait_psn = 0;
...
// static inline enum comp_state do_read(struct dtld_qp *qp,
// 				      struct dtld_pkt_info *pkt,
// 				      struct dtld_send_wqe *wqe)
// {
// 	int ret;
...
// 			payload_size(pkt), DTLD_TO_MR_OBJ);
// 	if (ret) {
// 		wqe->status = IB_WC_LOC_PROT_ERR;
// 		return COMPST_ERROR;
// 	}

// 	if (wqe->dma.resid == 0 && (pkt->mask & DTLD_END_MASK))
// 		return COMPST_COMP_ACK;

// 	return COMPST_UPDATE_COMP;
// }

// static inline enum comp_state do_atomic(struct dtld_qp *qp,
// 					struct dtld_pkt_info *pkt,
// 					struct dtld_send_wqe *wqe)
// {
// 	int ret;
...
// static void make_send_cqe(struct dtld_qp *qp, struct dtld_send_wqe *wqe,
// 			  struct dtld_cqe *cqe)
// {
...
//  * ---------8<---------8<-------------
//  */
// static void do_complete(struct dtld_qp *qp, struct dtld_send_wqe *wqe)
// {
// 	struct dtld_dev *dtld = to_rdev(qp->ibqp.device);
// 	struct dtld_cqe cqe;
// 	bool post;
...
// 	if (post)
// 		dtld_cq_post(qp->scq, &cqe, 0);

// 	if (wqe->wr.opcode == IB_WR_SEND ||
// 	    wqe->wr.opcode == IB_WR_SEND_WITH_IMM ||
// 	    wqe->wr.opcode == IB_WR_SEND_WITH_INV)
// 		dtld_counter_inc(dtld, DTLD_CNT_RDMA_SEND);
...
// static inline enum comp_state complete_ack(struct dtld_qp *qp,
// 					   struct dtld_pkt_info *pkt,
// 					   struct dtld_send_wqe *wqe)
// {
...
// static inline enum comp_state complete_wqe(struct dtld_qp *qp,
// 					   struct dtld_pkt_info *pkt,
// 					   struct dtld_send_wqe *wqe)
// {
...
// static void dtld_drain_resp_pkts(struct dtld_qp *qp, bool notify)
// {
// 	struct sk_buff *skb;
// 	struct dtld_send_wqe *wqe;
...
// static void free_pkt(struct dtld_pkt_info *pkt)
// {
// 	struct sk_buff *skb = PKT_TO_SKB(pkt);
// 	struct dtld_qp *qp = pkt->qp;
...
// 		dtld_drain_resp_pkts(qp, qp->valid &&
...
// 			} else {
// 				dtld_counter_inc(dtld, DTLD_CNT_RETRY_EXCEEDED);
