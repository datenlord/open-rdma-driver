...
static int dtld_modify_qp(struct ib_qp *ibqp, struct ib_qp_attr *attr, int mask,
			  struct ib_udata *udata)
{
    int err;
...
static int dtld_query_qp(struct ib_qp *ibqp, struct ib_qp_attr *attr, int mask,
...
{
    // TODO: not supported in kernel now, support when our userspace driver is
    // stable.
    return 0;
}
...
static int dtld_create_cq(struct ib_cq *ibcq,
...
static struct ib_mr *dtld_reg_user_mr(struct ib_pd *ibpd, u64 start, u64 length,
				      u64 iova, int access,
				      struct ib_udata *udata)
{
    int err;
...
int dtld_mmap(struct ib_ucontext *context, struct vm_area_struct *vma)
{
    ...
    err = rdma_user_mmap_io(context, vma, PFN_DOWN(dtld_ummap_ent->address),
			    PAGE_SIZE, prot, ummap_ent);
